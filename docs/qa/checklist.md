...
| # | Gr | Punto | Cómo se comprueba |
| --- | --- | --- | --- |
| 13.1 | B | **Ningún «hoy le toca» para un riego que nadie ha marcado**, para cualquier planta con `riego.calculable: false`. El conjunto se lee del JSON en cada pasada, no se fija aquí: el 11/08 eran `helecho`, `begonia-elatior` y `poto`, y a media sesión Carlos dio las fechas y pasaron a `calculable: true` | `tests/franja-hoy.js` → caso `riego afirmado sin dato`. No necesita atributos: la frase se juzga sola. **Ojo:** si ninguna planta está sin ancla, el punto no está verificado — `tests/autoprueba.js` lo declara `NO INYECTADO` en vez de darlo por bueno |
...
| 13.6 | A | **Ocupación: ninguna ficha con más del 20 % de sus bandas de 100 px donde el contenido pare antes del 62 % del ancho.** Es la métrica de «la mitad está vacía», y es la que de verdad estaba rota | `tests/expediente.js` → `bandas_cortas_pct`. Procedencia: `docs/brief.md` § «Los objetivos que sustituyen al de 2.400 px» (`ux-lead`, 4f1b350) |
| 13.7 | A | **Altura por severidad**, a 1280×900: **≤ 3 pantallas (2.700 px)** en `critica`/`atencion`, **≤ 2 (1.800 px)** en `sana`. Dos tramos a propósito: un único número obliga a que una planta sin problema y una que se muere quepan en lo mismo | `tests/expediente.js` → `pantallas` vs `tope_pantallas` |
| 13.8 | B | **Se mide en las SIETE, no en una.** La rejilla va ordenada por urgencia, así que `--abrir 0` abre siempre el helecho — y el helecho es el 6º de 7 por altura. Las fichas son `<details name="planta">`, acordeón exclusivo: hay que abrir-medir-restaurar una a una | `tests/expediente.js` lo hace solo; `fichas_medidas` tiene que decir 7 |
| 13.9 | M | El expediente pinta **dos columnas de tinta de verdad**, no un `grid` declarado y sin repartir | `tests/expediente.js` → `columnas_de_tinta` |
| 13.10 | A | **Todo eje con rótulos de valor sostiene su geometría**, no solo la cronología: tres ajustes (log / lineal / índice) y gana el mejor. Si gana el índice, el dibujo afirma proporciones que el dato no tiene. Los rótulos ordinales (1, 2, 3…) y los categóricos se abstienen — espaciarlos regular es correcto | `tests/diagramas.js` → `ejes[]`. Generalizado porque el diagrama de recuperación tenía este defecto y **pasó mi informe 2 en verde**: comprobé que existía, no que dijera la verdad |
...
| 13.14 | B | **Ningún objetivo de altura se cumple recortando contenido.** Se cumple ocupando el ancho, quitando rótulos falsos y borrando gráficos que no informan — nunca recortando observaciones, causas, límites ni fuentes. Si un objetivo obliga a borrar una observación de `botanist`, el que está mal es el objetivo | comparar `cobertura-datos` antes y después: el número de campos que llegan a pantalla no puede bajar. `docs/brief.md`, misma sección |
| 13.15 | A | Los diagramas tienen equivalente en texto (6.6) y `role`/`title` o `aria-hidden` coherente (6.5) | `tests/diagramas.js` → `inventario` |
